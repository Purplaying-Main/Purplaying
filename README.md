...
22.11.22
	...
	- [각자 맡은 페이지에 있으면..]파일첨부, 이미지업로드 기능
	=======================================================
	- 암호화(결제내역, 로그인정보) > 세션에 암호화되어서 저장
	- 소셜로그인
	- 유효성체크(각 페이지별)
	   ex. 프로젝트작성페이지: 목표금액 실시간계산, 등록전 총 리워드수량X금액비교(목표금액보다 많아야함)
	=======================================================
	- [이희라] 프로젝트 썸네일 파일첨부 작업중
	O [한승훈] 상세페이지 펀딩계획 자동계산 스크립트 완료 
	- [고영은] JOIN문으로 유저 테이블, 셋팅 테이블, 배송지 테이블 연결. ajax 작성중
	- [양승민] 테이블이 미출력. 테스트 중
	- [전완철] 테이블 JOIN을 위한 Mybatis ResultMap 테스트중
