...
<body>
				...
				<div class="row col-10 justify-content-center mx-auto">
					...
					<div class="tab-content" id="v-pills-tabContent">
						<div class="tab-pane fade show active py-5">
							...
							<form class="card p-5 mb-3" id="form" action="" method="post">
								...
								<div class="mt-3">
									...
									<small class="card-subtitle mb-2 text-muted"><fmt:formatDate
											value="${oneononeDto.inquiry_regdate}" pattern="yyyy-MM-dd"
											type="date" /></small> <small class="card-subtitle mb-2 text-muted">writer
										: ${oneononeDto.writer} | user_id : ${sessionScope.user_id}</small>
								</div>
								<hr class="my-4">
								<!-- 본문 영역 -->
								...
								<div class="mt-3 text-end">
									<!-- 수정권한 확인  -->
									<c:if
										test="${oneononeDto.writer eq sessionScope.user_id or sessionScope.user_id eq 'admin@gmail.com'}">
							...
							<input type="hidden" name="inquiry_no" value="${ansState}">
								...
								<form class="card p-5 mb-3" id="form" action="" method="post">
									...
									<div class="mt-3">
										...
										<small class="card-subtitle mb-2 text-muted" name="ans"><fmt:formatDate
												value="${ansDto.ans_regdate}" pattern="yyyy-MM-dd"
												type="date" /></small> <small class="card-subtitle mb-2 text-muted">writer
											: ${oneononeDto.writer} | user_id : ${sessionScope.user_id}</small>
									</div>
									<hr class="my-4">
									...
									<!--  	<button type="button" id="ans_modifyBtn" class="btn btn-outline-primary" onclick="showUpdateans_modiModalModal({{ansDto.ans_no}})"> -->
												<i class="fa fa-edit"></i>수정
											</button>
							...
							<div class="row mx-auto col-md-4">
								<button type="button" class="btn btn-outline-primary my-3" onclick="location.href='/purplaying/oneonone/list'">목록으로 돌아가기</button>
						<!--	<button type="button" class="btn btn-outline-primary my-3" onclick="history.back(-1)">목록으로 돌아가기</button>  -->	
							...
							<!-- Modal end-->
							<div class="modal fade" id="ans_modiModal" tabindex="-1"
								aria-labelledby="ansModiModalLabel" aria-hidden="true">
								<div class="modal-dialog">
									<div class="modal-content">
										<div class="modal-header p-5 pb-4 border-bottom-0">
										...
										<!-- 답변 수정 란 -->
											...
											<div class="form-floating">
											 ...
											 <div class="form-floating">
												<hr class="mb-4">
												<h5>문의 답변</h5>
												<textarea class="form-control mt-2 summernote"
													id="ansWriteContext">${ansDto.ans_context}</textarea>
											 </div>
	...
	$(document).ready(function() {
		...
		$("#ansModiComplete").click(function() {
		 	let ansmodiData = {ans_context:$('#ansWriteContext').val(),ans_no:$('#ans_no').val()}
			$.ajax({
	            type : 'post',										// 요청 메서드
		...
		<!--
		//ans_modiComplete
		$("#ans_modiComplete").on("click", function() {
			let ansData = {ans_context:$('#ansWriteContext').val(),inquiry_no:$('#inquiry_no').val()}
			$.ajax({
	            type : 'PATCH',										// 요청 메서드
	            url : '/purplaying/oneonone/write/ans',								// 요청 URI
		...
		//ans_removeBtn
		$("#ans_removeBtn").on("click", function() {
			let ansData = {ans_context:$('#ansWriteContext').val(),inquiry_no:$('#inquiry_no').val()}
			$.ajax({
	            type : 'post',										// 요청 메서드
	            url : '/purplaying/oneonone/write/ans',								// 요청 URI
